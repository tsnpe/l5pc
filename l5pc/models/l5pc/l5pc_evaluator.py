...
from multicompartment.models.l5pc import l5pc_model  # NOQA
from multicompartment.models.l5pc.utils import return_protocol_subset
...
def define_protocols(protocol_subset: Optional[str] = None):
    """Define protocols"""

    protocol_definitions = json.load(open(os.path.join(config_dir, "protocols.json")))
    protocol_definitions = return_protocol_subset(protocol_definitions, protocol_subset)
...
def define_fitness_calculator(protocols, protocol_subset: Optional[str] = None):
    """Define fitness calculator"""

    feature_definitions = json.load(open(os.path.join(config_dir, "features.json")))
    feature_definitions = return_protocol_subset(feature_definitions, protocol_subset)
